...
onMounted(async () => {
  ...
  let total = await axiosUtilObj.postToken("/api/auth/blog/getFavoritesTotalCount", store.data.token, null)
  totalItem.value = total.data

  let list = await axiosUtilObj.postToken("/api/auth/blog/getFavoritePostList", store.data.token, {currentPage: currentPage.value})
  console.log(list.data)
  if (list.data !== undefined) {
    data.list = list.data

    for (let i = 0; i < list.data.length; i++) {
      data.list[i].publishedAt = timeUtilObj.StrToFormatDate(list.data[i].publishedAt)

      let authorId = list.data[i].userId
...
const handleCurrentChange = async (val) => {
  let list = await axiosUtilObj.postToken("/api/auth/blog/getFavoritePostList", store.data.token, {currentPage: val})
  if (list.data !== undefined) {
    data.list = list.data
