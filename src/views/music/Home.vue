...
onBeforeMount(async () => {
  let songList = await axiosUtilObj.postToken("/api/auth/music/list", store.data.token, null)
  if (songList.data !== undefined) {
    ...
    let songFavoriteList = await axiosUtilObj.postToken("/api/auth/music/favoriteList", store.data.token, null)
    let tempMap = new Map()
    if (songFavoriteList.data !== undefined) {
      for (let v of songFavoriteList.data) {
        tempMap.set(v, true)
      }
    }
    for (let i = 0; i < songList.data.length; i++) {
      if (tempMap.has(songList.data[i].id)) {
        data.listFavoriteMap.set(songList.data[i].id, true)
      } else {
        data.listFavoriteMap.set(songList.data[i].id, false)
      }
    }
  }
})
